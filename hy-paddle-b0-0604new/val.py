...
from utils.utils import ConfusionMatrix, postprocess, scale_coords, process_batch, ap_per_class, fitness, \
    DataLoaderX, BBoxTransform, ClipBoxes, boolean_string, Params
...
@paddle.no_grad()
def val(model, val_generator, params, opt, seg_mode, is_training, img_transform, **kwargs):
    model.eval()
    ...
    iou_thresholds = paddle.linspace(0.5,0.95,10)
    num_thresholds = iou_thresholds.numel().item()
    ...
    for iter, data in enumerate(val_loader):
        ...
        if opt.cal_map:
            out = postprocess(imgs.detach(),
                              paddle.stack([anchors[0]] * imgs.shape[0], axis=0).detach(), regression.detach(),
            ...
            for i in range(annot.shape[0]):
                ...
                pred = paddle.to_tensor(pred, dtype=paddle.float64)

                target_class = labels[:, 4].tolist() if nl else []  # target class

                if len(pred) == 0:
                    if nl:
                        stats.append((paddle.zeros(0, num_thresholds, dtype=paddle.bool),
                                      paddle.Tensor(), paddle.Tensor(), target_class))
                    # print("here")
                    continue
                ...
                if nl:
                    ...
                    correct = paddle.zeros([pred.shape[0], num_thresholds], dtype=paddle.bool)
                # stats.append((correct.cpu(), pred[:, 4].cpu(), pred[:, 5].cpu(), target_class))
                stats.append((correct, pred[:, 4], pred[:, 5], target_class))
...
# 看到这里
            ...
            if seg_mode == MULTICLASS_MODE:
                ...
                segmentation = F.log_softmax(segmentation, axis=1)
                segmentation = segmentation.exp()
                # _, segmentation = torch.max(segmentation, 1)  # (bs, C, H, W) -> (bs, H, W)
                segmentation = paddle.argmax(segmentation, axis=1)
            else:
            ...
            for i in range(ncs):
                iou_ls[i].append(iou.t()[i].detach().numpy())
                acc_ls[i].append(acc.t()[i].detach().numpy())
    ...
    if opt.cal_map:
        ...
        if is_training and fi > best_fitness:
            ...
            # savepath = opt.saved_path + f'hybridnets-d{opt.compound_coef}_{epoch+1}_{step-1}_with_bestmap.pdparams'
        ...
        if is_training and loss + opt.es_min_delta < best_loss:
            best_loss = loss
            best_epoch = epoch
            # savepath = opt.saved_path + f'hybridnets-d{opt.compound_coef}_{epoch+1}_{step-1}_with_bestloss.pdparams'
            # obj = {'model': model.state_dict(), 'opt': optimizer.state_dict(), 'epoch': epoch+1, 'step':step-1}
            # paddle.save(obj, savepath)  
...
if __name__ == "__main__":
    ...
    try:
        model.load_state_dict(paddle.load(weights_path))
    except:
        model.load_state_dict(paddle.load(weights_path)['model'])
