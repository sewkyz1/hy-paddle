...
def get_args():
    ...
    parser.add_argument('-n', '--num_workers', type=int, default=0, help='Num_workers of dataloader')  # wimdows不支持多workers
    parser.add_argument('-b', '--batch_size', type=int, default=2, help='Number of images per batch among all devices')
    parser.add_argument('--freeze_backbone', type=boolean_string, default=True,
...
def train(opt):
    ...
    if opt.load_weights:  # 导入权重
        ...
        try:
            ...
            model.set_state_dict(ckpt.get('model', ckpt), use_structured_name=False)
    ...
    if opt.freeze_backbone:  # 是否停止训练这一部分
        for i, param in enumerate(model.encoder.named_parameters()):  # 返回所有最底层的参数nn.Conv2D，nn.BatchNorm2D，nn.Linear
            # print(param)
            param[1].trainable = False
            # print(param)
        for i, param in enumerate(model.bifpn.named_parameters()):  # 返回所有最底层的参数nn.Conv2D，nn.BatchNorm2D，nn.Linear
            # print(param)
            param[1].trainable = False
            # print(param)
    ...
    if opt.freeze_det: 
        for i, param in enumerate(model.regressor.named_parameters()):  # 返回所有最底层的参数nn.Conv2D，nn.BatchNorm2D，nn.Linear
            # print(param)
            param[1].trainable = False
            # print(param)
        for i, param in enumerate(model.classifier.named_parameters()):  # 返回所有最底层的参数nn.Conv2D，nn.BatchNorm2D，nn.Linear
            # print(param)
            param[1].trainable = False
            # print(param)
        for i, param in enumerate(model.anchors.named_parameters()):  # 返回所有最底层的参数nn.Conv2D，nn.BatchNorm2D，nn.Linear
            # print(param)
            param[1].trainable = False
            # print(param)
    ...
    if opt.freeze_seg:
        for i, param in enumerate(model.bifpndecoder.named_parameters()):  # 返回所有最底层的参数nn.Conv2D，nn.BatchNorm2D，nn.Linear
            # print(param)
            param[1].trainable = False
            # print(param)
        for i, param in enumerate(model.segmentation_head.named_parameters()):  # 返回所有最底层的参数nn.Conv2D，nn.BatchNorm2D，nn.Linear
            # print(param)
            param[1].trainable = False
            # print(param)
            ...
            for iter, data in enumerate(progress_bar):
                ...
                try:
                    ...
                    with paddle.amp.auto_cast(enable=opt.amp):
                        ...
                        cls_loss = cls_loss.mean() if not opt.freeze_det else paddle.to_tensor(0)
                        reg_loss = reg_loss.mean() if not opt.freeze_det else paddle.to_tensor(0)
                        seg_loss = seg_loss.mean() if not opt.freeze_seg else paddle.to_tensor(0)
